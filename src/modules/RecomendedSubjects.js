...
const meetPrerequisite = (subjectToEvaluate) => {
  ...
  if (!isUnlocked) {
    isUnlocked = ALL_SUBJECTS.some((subject) => {
      const IS_PASSED =
        subject.status === VALIDATED || subject.status === APPROVED;
      const MATCH_PREREQUISITE =
        subjectToEvaluate.prerequisite === subject.code;
...
const isNotPassed = (subjectToEvaluate) => {
  return (
    subjectToEvaluate.status === PENDING || subjectToEvaluate.status === FAILED
  );
};
...
const addToRecommendedSubjects = (subjectToAdd, impact) => {
  const POSITION = RECOMMENDED_SUBJECTS.findIndex(
